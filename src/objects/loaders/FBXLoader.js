...
class FBXLoader extends THREE.Loader {
	 ...
	 * @param {function(THREE.Group)} onLoad - Executed when the loading process has been finished.
	 ...
	 * @return {THREE.Group} An object representing the parsed asset.
	 */
...
class FBXTreeParser {
	...
	parseTextures( images ) {
		...
		if ( 'THREE.Texture' in fbxTree.Objects ) {
	...
	parseParameters( materialNode, textureMap, ID ) {
		...
		} else if ( materialNode.DiffuseColor && ( materialNode.DiffuseColor.type === 'THREE.Color' || materialNode.DiffuseColor.type === 'ColorRGB' ) ) {
		...
		} else if ( materialNode.EmissiveColor && ( materialNode.EmissiveColor.type === 'THREE.Color' || materialNode.EmissiveColor.type === 'ColorRGB' ) ) {
		...
		} else if ( materialNode.SpecularColor && materialNode.SpecularColor.type === 'THREE.Color' ) {
	...
	// create a THREE.PerspectiveCamera or OrthographicCamera
...
class GeometryParser {
	...
	genFace( buffers, geoInfo, facePositionIndexes, materialIndex, faceNormals, faceColors, faceUVs, faceWeights, faceWeightIndices, faceLength ) {
		...
		if ( faceLength > 3 ) {
			...
			// resulting in an array of 0 triangles being returned from THREE.ShapeUtils.triangulateShape
...
class AnimationParser {
	...
	// hierarchy. Each Stack node will be used to create an THREE.AnimationClip
	...
	generateRotationTrack( modelName, curves, preRotation, postRotation, eulerOrder ) {
		...
		// For Maya models using "Joint Orient", THREE.Euler order only applies to rotation, not pre/post-rotations
...
class BinaryParser {
	...
	parseSubNode( name, node, subNode ) {
		...
		if ( subNode.singleProperty === true ) {
			...
			if ( innerPropType1 === 'THREE.Color' || innerPropType1 === 'ColorRGB' || innerPropType1 === 'Vector' || innerPropType1 === 'Vector3D' || innerPropType1.indexOf( 'Lcl_' ) === 0 ) {
...
module.exports = exports = FBXLoader;
